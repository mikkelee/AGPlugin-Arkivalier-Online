...
+ (NSXMLNode*) getNode: (NSXMLNode*) element withName: (NSString*) name {
	...
	for(NSXMLNode* child in [element children]) {
		NSXMLNode* el = [Soap getNode: (NSXMLElement*)child withName: name];
		if(el != nil) { return el; }
	}
	return nil;
}
...
+ (NSArray*) findNodes: (NSXMLNode*) node xpath: (NSString*) xpath {
	NSError* error;
	return [node nodesForXPath: xpath error: &error];
}

// Finds a single node with the given XPath query.
+ (NSXMLNode*) findNode: (NSXMLNode*) node xpath: (NSString*) xpath {
	NSArray* a = [Soap findNodes: node xpath: xpath];
	if(a != nil && [a count] > 0) {
		return (NSXMLNode*)[a objectAtIndex:0];
	}
	return nil;
}

// Deserialize an object as a generic object
+ (id) deserialize: (NSXMLNode*) element{
...
+(id)deserializeAsDictionary:(NSXMLNode*)element {
	if([element childCount] == 1) {
		NSXMLNode* child = [[element children] objectAtIndex:0];
...
+ (id) deserialize: (NSXMLNode*) element forObject: (NSObject*) object {
	NSError* error;
	NSObject* value;
...
+ (NSString*) getNodeValue: (NSXMLNode*) node withName: (NSString*) name {
	...
	{
		child = [(NSXMLElement*)node attributeForName: name];
		if(child != nil) {
...
+(id)objectFromNode:(NSXMLNode*)node {
	NSMutableArray* children = [NSMutableArray arrayWithArray:[node children]];
	if([node isKindOfClass:[NSXMLElement class]]) {
		[children addObjectsFromArray:[(NSXMLElement*)node attributes]];
	}
	if([children count] > 0) {
