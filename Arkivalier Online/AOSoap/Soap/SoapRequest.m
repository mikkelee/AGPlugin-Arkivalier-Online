...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
	...
	NSXMLDocument* doc = [[NSXMLDocument alloc] initWithData: self.receivedData options: 0 error: &error];
	if(doc == nil) {
		[self handleError:error];
		return;
	}
	...
	if([fault hasFault]) {
		...
		NSXMLNode* element = [[Soap getNode: [doc rootElement] withName: @"Body"] childAtIndex:0];
